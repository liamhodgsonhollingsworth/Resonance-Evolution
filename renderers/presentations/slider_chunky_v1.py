"""slider_chunky_v1 — chunky raster slider variant."""
...
DEFAULT_TRACK = [0.22, 0.24, 0.30]
DEFAULT_THUMB = [0.75, 0.82, 0.92]
DEFAULT_OUTLINE = [0.95, 0.97, 1.0]
...
def render(input: dict) -> np.ndarray:
    ...
    if orientation == "vertical":
        thumb_w = max(8, int(round(width * 0.95)))
        thumb_h = max(8, int(round(height * 0.25)))
        thumb_x = (width - thumb_w) // 2
        max_thumb_y = max(0, height - thumb_h)
        thumb_y = int(round((1.0 - fraction) * max_thumb_y))
    else:
        thumb_w = max(8, int(round(width * 0.25)))
        thumb_h = max(8, int(round(height * 0.95)))
