...
## 2026-05-18 addendum — Phase 3 landed

Phase 3 (end-to-end chat-driven code generation) shipped via the workflow shell at `tools/workflow/`. The seven steps the original plan named are now operational against the fake-claude fixture in tests and against real `claude` subprocesses in interactive use:

1. The viewer types into the workflow shell. (Implemented: `python -m tools.workflow`.)
2. Bare text routes to the active session via SessionManager.send. (Implemented in `tools/workflow/shell.py`.)
3. Slash command `/wish <description>` packs a structured prompt naming the file-watcher's hot-reload guarantee. (Implemented; `_build_wish_prompt` in `tools/workflow/shell.py`.)
4. The session writes a new file under `node_types/<short>.py`. (Verified by the fake-claude `WRITE_NODE_TYPE` directive in tests; real sessions write files directly via the standard Edit/Write tools under `--permission-mode auto`.)
5. The file-watcher detects the file and calls `_load_node_type_file` (new file) or `reload_type` (modified file). (Already implemented; the workflow shell wires `FileWatcher(engine, on_event=shell.on_file_event)` at boot.)
6. The shell prints `[fwatch new TestClock <path>]` so the maintainer sees the registration land. (Implemented.)
7. `/dispatch spawn TestClock` (or any text-API mutation) spawns the new node-type. (Implemented; the workflow shell delegates to `tools/text_test::dispatch_command`.)

The remaining open questions named in the original plan stand: security model for autogenerated code (single-user mode is acceptable now), state migration on reload (still ignored at v1), MCP call latency inside emit() (MCPSource now does the call at precompute time, so per-frame cost is sub-ms), and what "workflow context" means spatially (the text-rendering of the workflow shell is the v1 answer; the 3D rendering arrives when wishlist #023 lands).

The cockpit (TypeScript Electron app at `Alethea/.claude/worktrees/nervous-lalande-93b4ea/cockpit/`) is an *alternative rendering* of the same surface using the same primitives (Session / MessageBus / Panel / SkillRegistry / Replica). When the cockpit merges and the realtime renderer lands, all three (text shell, cockpit, 3D scene) drive the same engine.
