...
| `connector` | the scope's far endpoint is an **external** system (§2.4) — the in-game chat seam with a SELECTABLE `channel` (in_world / dev_console / external_bridge) over the canonical envelope, a dumb delegate on `runtime/comm_channel.gd` | **shipped** |
...
`abstract` cache; the `connector` handler realizes §2.4 as an in-game chat seam with a SELECTABLE
`channel` — in_world / dev_console / external_bridge — a dumb delegate over the transport-neutral narrow
waist in `runtime/comm_channel.gd`, the existing `bridge/` mechanism reused for the external channel).
The edge-level `Channel` module with capacity/backpressure and the remaining abstraction extensions
(§2.5) are the sequenced follow-ons — each a new module, not a foundation edit.
