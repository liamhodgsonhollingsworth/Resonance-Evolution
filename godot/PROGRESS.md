...
**Generalized probabilistic WFC — the weight oracle + evolving distributions (NEW 2026-07-02, verified).** The `wfc` Context handler is generalized per `notes/design/generalized_probabilistic_wfc_2026-07-01.md` (implemented exactly, no further): `primitives/wfc_generalized.gd` supplies the weight ORACLE `W(tile, cell, ctx)` in three tiers — T0 uniform (the base case), T1 static-conditional rules (bias a tile beside a committed neighbour), T2 EVOLVING distributions (weight = a closed data-only `weight_expr` — const/linear/inverse/quota/sum/product/ref — over generation-state counters `tile_count`/`adjacent_pair`/`region_count`/`run_length`; P(A|B) grows/decays with the committed n_AB — the spec's exact example). Plus opt-in weighted Shannon entropy (dirty-set cached), soft-STARVATION counting + `min_weight` floor, and opt-in BACKTRACKING (`backtrack`/`backtrack_limit`, `exhausted` on budget-exceed; never hangs). Everything is DATA in `params.wfc` (all new fields optional); the `prim_context.gd` seam is +38/−3 and base rulesets run the untouched deterministic path. HARD INVARIANT mechanized: uniform/static configs routed through the generalized path are BYTE-IDENTICAL to the base handler (4 rulesets x 5 seeds, grid + contradiction + collapses). Test: `godot --headless --path godot -s res://headless_wfc_prob_test.gd` (21/21, **RESULT: ALL PASS**; PERF 30x30 base=217ms gen-uniform=231ms, 50x50 base=1633ms gen-uniform=1548ms, 30x30 T2+weighted-entropy=4684ms). Base suite `headless_wfc_test.gd` still ALL PASS verbatim. Adversarial findings recorded in the design note's new §8 appendix. Deferred as staged: pins + the Phase-4 authoring GUI (supervised).
