"""RightSidebarNode — vertical bar variant of BarNode anchored to the
right.

Brief 04 commit 2 of the Resonance website implementation arc. Sister
variant to LeftSidebarNode; same shape, different default anchor. See
``node_types/bar.py`` + ``node_types/left_sidebar.py`` for the design
...
def manifest() -> Manifest:
    return _bar_module._make_manifest("RightSidebarNode")


def build(params: Dict[str, Any]) -> Dict[str, Any]:
    return _bar_module._build_with_kind("RightSidebarNode", params)
