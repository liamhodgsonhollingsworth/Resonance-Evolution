...
- **#025** [pending] — **Full-screen interactive mode.** The renderer takes over the screen; toggleable via Escape with the WorkflowView. Realtime renderer landed (#023) provides the window; the fullscreen-toggle hook is queued as a small follow-up — `TkBackend.set_fullscreen(bool)` calling `root.attributes("-fullscreen", True)` plus an F11 binding in the realtime driver's global handlers.
